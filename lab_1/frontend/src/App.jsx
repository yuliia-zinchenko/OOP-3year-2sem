        ...
        {loans.length === 0 ? (
          ...
          <ul className="list">
            {loans.map((l) => (
              <li key={l.id}>
                <div>
                  <div>Книга #{l.bookId} <span className={`status status-${l.status}`}>{statusLabel(l.status)}</span></div>
                  <div className="meta">
      ...
      {orders.length === 0 ? (
        ...
        <ul className="list">
          {orders.map((l) => (
            <li key={l.id}>
              <div>
                <div>Замовлення #{l.id}</div>
                <div className="meta">Читач {l.userId} · книга #{l.bookId}</div>
              </div>
